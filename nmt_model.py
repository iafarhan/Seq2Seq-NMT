...
class NMT(nn.Module):
    ...
    def forward(self, source: List[List[str]], target: List[List[str]]) -> torch.Tensor:
        ...
        if self.charDecoder is not None:
            ...
            ### very  imp ###
            target_outputs = combined_outputs.view(-1, 256) #combined output vector at current timestep is used as well as oov states
            ...
            #its very important to pass the decoder init states coz these are actually the oov hidden states
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        Y = self.model_embeddings_target(target_padded) #(target_len, batch, emb size)
